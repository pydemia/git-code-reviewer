...
신규 저장소의 첫 연결에서는 worker가 초기 지식을 발행하는 동안 `unavailable`이 발생할 수 있다. 현재 클라이언트는 이 발행을 자동으로 기다리지 않으므로 잠시 후 같은 key와 설정으로 다시 연결한다. PRISM-DEV 검증에서는 최초 실패 뒤 약 11초 후 manifest가 준비됐다. 이 시간은 보장값이 아니다.
