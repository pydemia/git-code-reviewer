...
[GitHub PR 조회 API](https://docs.github.com/en/rest/pulls/pulls#get-a-pull-request)의 head를 확인한 뒤 [issue comment 갱신 API](https://docs.github.com/en/rest/issues/comments#update-an-issue-comment)를 호출한다. 후자의 요청 매개변수에는 PR head와의 원자적 비교 조건이 없다. 따라서 마지막 조회와 쓰기 사이의 변경 가능성은 남는다. 이 검증은 그 시간 간격을 줄이며 원격 쓰기를 atomic CAS로 만들지는 않는다. 댓글에는 분석 당시 SHA가 계속 표시된다.
