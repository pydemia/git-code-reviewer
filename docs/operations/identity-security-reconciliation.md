...
P03-C05 개발 기록이다. 현재 PRISM-DEV 배포는 P03-C04의 alpha.36이며 아래 수집기는 아직 배포하지 않았다. 관리자 차단·재활성화·전체 기기 로그아웃 API와 UI, 운영 companion 구성 검증을 마친 뒤 C05 완료 여부를 판정한다.
...
남은 작업은 확인된 재활성화와 전체 기기 로그아웃 API/UI, 활성화 작업과 수집기의 경합, 운영 설정·부하·복구 검증, 최종 회귀와 배포다. Native Safari·VS Code 검증도 별도 항목으로 유지한다.
