...
현재 근거는 저장된 보고서와 메모리입니다. 질문별로 저장소 코드를 새로 조회하거나 base/head 구현과 관련 테스트를 추가 탐색하는 고급 Chat은 구현 계획 단계입니다. 관련 코드 링크가 있다는 이유만으로 모델이 해당 파일 전체를 읽었다고 판단하지 않습니다.
...
현재 제공 범위는 PR snapshot 분석, 보고서·코드 탐색, 보고서 기반 Chat, 개인·집단 메모리와 PR 대화 수집·관리입니다.

질문별 base/head 원본 범위 조회, 호출 관계와 관련 테스트의 추가 탐색, 답변별 상세 context manifest, 긴 질문을 위한 비동기·streaming 실행은 고급 Chat backlog에 남아 있습니다. 현재 화면의 Tests는 정적 해석이며 저장소의 빌드나 테스트 코드를 실행하지 않습니다.
