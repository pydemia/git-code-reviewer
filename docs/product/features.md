...
왼쪽 목록에서 PR을 선택하면 오른쪽에 작성자·파일 위치·답글을 묶은 대화가 표시됩니다. 제목은 불러온 목록에서 검색하고, 과거 PR은 번호를 입력한 뒤 **PR 번호로 열기**로 바로 조회합니다. 저장된 이력만 조회하며 새 수집을 요청하지 않습니다.

코멘트의 **원문·변경 이력 보기**를 누르면 전체 Markdown 본문이 해당 대화 안에서 펼쳐집니다. 표·목록·인용·코드를 읽거나 **Markdown 원문 텍스트**로 저장된 텍스트를 확인할 수 있습니다. 이전 본문은 **수정·관측 이력 확인**, 출처 지침은 **이 원문에 연결된 지침**에서 확인합니다. 외부 이미지와 HTML은 자동으로 실행·로드하지 않습니다.
