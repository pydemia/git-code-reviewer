...
Git Code Reviewer에 tenant 격리, 일반 사용자와 관리자 구분, 사용자 관리, RBAC+ABAC 인가와 tenant별 code analysis prompt 관리를 추가한다. 인증과 인가를 application code에 중복 구현하지 않고 다음 책임으로 분리한다.

| 영역 | 정본 | 책임 |
|---|---|---|
| Identity | Keycloak 또는 조직 표준 OIDC provider | 로그인, MFA/SSO/session, identity lifecycle, signed role/group claim |
| Application access | PostgreSQL | tenant, local user 상태, tenant membership, repository tenant, prompt version |
...
| `administrator` | tenant, membership, user 상태, repository와 analysis prompt를 관리하고 모든 tenant resource 조회 |
...
Prompt version은 삭제하거나 수정하지 않는다. Tenant마다 active version은 최대 하나이며 reset하면 built-in instruction으로 돌아간다. Report에는 prompt 원문이 아니라 version과 hash만 기록한다.
...
Snapshot materialization이 analysis run을 만들 때 repository tenant의 active prompt를 선택하고 `prompt_version_id`와 `prompt_hash`를 고정한다. Worker 시작 뒤 active prompt가 바뀌어도 해당 run은 기존 version을 사용한다.
...
- Admin은 `Tenants`, `Users`, `Analysis prompt` tab을 사용한다.
