...
작성: 2026-09-19. PR #1024의 PRISM-DEV 저장 결과와 alpha.67 코드를 대조했다. 이 문서는 초기 구현 계획이다. 후속 구현·검증의 범위와 남은 제한은 [실행 기록](execution/large-pr-analysis/2026-09-19.md)에 구분해 기록한다. 조사 시점에는 새 PR 생성, 모델 재호출, 외부 PR 댓글 게시를 수행하지 않았다.
...
## 2026-09-19 공유 계정 기준 보정

실제 #1024의 계정 usage limit 관측에 따라 대형 PR 기본 호출 예산을 128회로 조정했다. 실행 총량은 요청 속도와 구분하며 계정 공통 분석 동시성 최대 2개, 기존 분당 요청·입력량 예산의 절반을 분석에 사용한다. 공급자의 실제 계정 잔여량은 관측하지 못하므로 외부 클라이언트와의 무충돌을 보장하지 않는다. quota와 deadline을 분리하고 완료 결과를 유지한 명시적 재개를 사용한다. 현재 원인·수치·검증은 [공유 계정 실행 기록](execution/large-pr-analysis/2026-09-19-shared-account.md)에 남겼다.

2026-09-19 후속 점검에서 같은 등록 계정의 읽기 전용 usage API로 주간 사용량 100%와 19:32 KST reset을 직접 확인했다. PRISM-DEV의 실행당 예산은 요청에 따라 256회로 늘리되 계정 cooldown·동시성·분당 제한을 유지한다. 계정 제한이 풀리기 전에는 재호출하지 않는다. GitHub 코드 참조와 큰 게시 문안의 부분 생략 보정, 최종 배포·재개 상태는 [후속 실행 기록](execution/large-pr-analysis/2026-09-19-code-references.md)에 남긴다.
