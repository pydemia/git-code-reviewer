...
리뷰 이력은 상단 제목·저장소 선택, 왼쪽 PR 탐색 목록, 오른쪽 선택 PR의 대화로 구성한다. PR 번호·제목 검색은 현재 불러온 목록에만 적용하며 범위를 명시한다. 페이지네이션은 유지한다. 작성자·날짜·유형·파일 위치가 본문 위에 있고 부모가 로드된 답글은 같은 스레드 안에서 표현한다. 원문이 없는 부모나 일부 페이지의 답글을 완전한 스레드처럼 표시하지 않는다.
