...
- 요약은 개조식으로 간결하게 쓴다. 논점이 나뉠 때만 `## 검토 의견`, `## 다음 조치` 같은 Header를 두고 아래에 List를 배치한다. 내용이 짧으면 제목 없이 바로 결론을 쓴다.
...
- 원인·실행 흐름·예외 조건의 자세한 설명은 문단으로 유지한다. 읽기 쉽게 줄이더라도 판단에 필요한 근거와 제한은 생략하지 않는다.
