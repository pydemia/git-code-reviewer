...
- 요약할 내용은 개조식으로 간결하게 쓴다. 내용이 구분될 때만 `## 주요 변경`, `## 검토 의견`, `## 다음 조치` 같은 Header와 List를 사용하며 빈 section이나 형식적인 항목은 만들지 않는다.
...
- 원인·실행 흐름·trade-off를 자세히 설명해야 한다면 문단으로 작성한다. 요약을 짧게 만들기 위해 발생 조건·근거·분석 제한을 제거하지 않는다.
